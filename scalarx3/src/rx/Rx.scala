...
trait Rx[+T] {
  ...
  private[rx] def downStream: mutable.Set[Rx.Dynamic[?]]
...
object Rx {
  ...
  def unsafe[T](func: => T): Rx[T] = ??? // todo: understand why this is needed
  ...
  private[rx] implicit val ordering: Ordering[Dynamic[?]] = Ordering.by[Rx.Dynamic[?], Int](-_.depth)

  @inline private[rx] def doRecalcCopy(rxs: Iterable[Rx.Dynamic[?]], obs: Iterable[Obs]): Unit = {
    doRecalcMutable(rxs.to(mutable.PriorityQueue), obs.to(mutable.Set))
  }
  private[rx] def doRecalcMutable(queue: mutable.PriorityQueue[Rx.Dynamic[?]], observers: mutable.Set[Obs]): Unit = {
    val seen = mutable.Set.empty[Rx.Dynamic[?]]
    var currentDepth = 0
  ...
  class Dynamic[+T](func: (Ctx.Owner, Ctx.Data) => T, owner: Option[Ctx.Owner]) extends Rx[T] { self =>
    ...
    private[rx] val downStream = mutable.Set.empty[Rx.Dynamic[?]]
    private[rx] val observers = mutable.Set.empty[Obs]

    private[rx] var cached: Try[T @uncheckedVariance] = uninitialized

    private[rx] var depth = 0
    private[rx] var dead = false
    private[rx] val upStream = mutable.Set.empty[Rx[?]]
    private[rx] val owned = mutable.Set.empty[Rx.Dynamic[?]]
